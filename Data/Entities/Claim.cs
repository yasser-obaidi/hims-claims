...
internal class ClaimConfiguration : IEntityTypeConfiguration<Claim>
{
    ...
    {
        builder.HasOne(x=>x.NetworkProvider)
            .WithMany(NetworkProvider=>NetworkProvider.Claims)
            .HasForeignKey(x=>x.NetworkProviderId)
            .OnDelete(DeleteBehavior.Restrict);

        builder.HasOne(x => x.TPA)
           .WithMany(TPA => TPA.Claims)
           .HasForeignKey(x => x.TPAId)
           .OnDelete(DeleteBehavior.Restrict);
    }
}